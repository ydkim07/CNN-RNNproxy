...
dummy = 20  # added to water PRD sample to handle loss calculation
grid_Nx = 40
grid_Ny =40
grid_Nz = 8

# Arrange data -local GPU
Permtraining = np.loadtxt('Permtraining.csv', delimiter=',', dtype = np.float32)
...
# Permdata = Permtraining[:,0:grid_Nx*grid_Ny]
...
while iteration < n_iterations and Termloss > trainTerminationLoss:
  history = model.fit((PermtrainX,BHPtrainX),trainY,batch_size=1000,verbose=0,epochs=200,validation_data=((PermdevX,BHPdevX),devY))
